...
export function NewsWidget({
  ...
  infoTooltipContent = 'Real-time financial news from major market sources. Stay updated on breaking news, market analysis, and company announcements that could affect your portfolio and trading decisions.',
  className = ''
}: NewsWidgetProps) {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        // Use different endpoints based on whether it's symbol-specific or general market news
        ...
        // Transform the data to our format (API might return array directly or wrapped)
        ...
        setNews(newsItems.slice(0, maxItems));
      } catch (err) {
    ...
    return (
      <div className={`bg-white rounded-lg border border-gray-200 shadow-sm p-6 ${className}`}>
        <h2 className="text-lg font-manrope font-semibold mb-4 flex items-center">
          {title}
          {showInfoTooltip && <InfoTooltip content={infoTooltipContent} />}
        </h2>
    ...
    return (
      <div className={`bg-white rounded-lg border border-gray-200 shadow-sm p-6 ${className}`}>
        ...
        <div className="text-center py-8 text-gray-500 text-sm">
          <p>{error ? 'Unable to load news' : `No news available${symbol ? ` for ${symbol}` : ' at this time'}`}</p>
