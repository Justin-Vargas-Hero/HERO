...
export default function MarketPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = marketSyncManager.subscribe('market-indices', () => {
      console.log('Market indices sync update triggered');
      fetchIndices();
    });
  ...
  useEffect(() => {
    ...
    // Subscribe to synchronized updates at :00 seconds
    const unsubscribe = marketSyncManager.subscribe('market-movers', () => {
      console.log('Market movers sync update triggered');
      loadMarketMovers();
    });
    ...
    const loadCalendarEvents = async () => {
      try {
        const [earningsRes, dividendsRes, ipoRes] = await Promise.all([
          fetch('/api/market/calendar/earnings?exchange=NYSE_NASDAQ'), // Filter to NYSE and NASDAQ
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
          {indicesLoading ? (
            ...
            const formatIndexPrice = (price: number) => {
              ...
              if (isCrypto) {
                // Crypto gets proper comma formatting
                return formatNumber(price, price > 10000 ? 0 : 2);
              }

              if (isIndex) {
                // Indices get comma formatting
            ...
            const formatIndexChange = (change: number, percent: number) => {
              ...
              const sign = change >= 0 ? '+' : '';
              const changeStr = `${sign}${formatNumber(Math.abs(change), 2)}`;
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-right">
                            <p className="font-inter text-sm font-medium">{formatCurrency(price)}</p>
